...
const App: React.FC = () => {
  ...
  const resetGame = async () => {
    ...
    // Interstitial Logic: Trigger every 4 full game restarts
  ...
  const handleContinueRequest = async () => {
    ...
    // Rewarded Logic: Always request a rewarded ad to continue
    await adService.prepareRewarded();
    setIsAdLoading(false);

    const rewardEarned = await adService.showRewarded();
    if (rewardEarned) {
      onAdFinished(); 
    } else {
      // If native failed (browser), show web fallback
      // The web fallback 'onClose' will call onAdFinished
      setShowAd(true);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-transparent text-white flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
        ...
        <div className="mb-12 space-y-4 flex flex-col items-center">
          <h1 className="text-7xl md:text-8xl font-black italic tracking-tighter drop-shadow-[0_10px_10px_rgba(0,0,0,0.5)] select-none animate-bounce px-8 overflow-visible" style={{ backgroundImage: 'linear-gradient(to right, #ff0000, #ff7f00, #ffff00, #00ff00, #0000ff, #4b0082, #8b00ff)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', animationDuration: '3s' }}>2048STACK</h1>
          <p className="text-xl font-medium text-white tracking-wide">Stack 'em high, don't cross the line!</p>
        </div>
        <div className="relative group mb-12">
          ...
          <button onClick={handleStartGame} className="relative px-12 py-5 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl font-black text-2xl shadow-2xl transition-all hover:scale-110 active:scale-95 flex items-center gap-4 border border-white/20"><i className="fas fa-play"></i> START GAME</button>
        </div>
        ...
        <button onClick={() => setShowPrivacy(true)} className="text-[10px] uppercase font-black tracking-widest text-white hover:text-white transition-colors flex items-center gap-2"><i className="fas fa-shield-halved"></i> Privacy & Terms</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-transparent text-white flex flex-col items-center p-4 animate-in zoom-in-95 duration-500">
      {showAd && <AdInterstitial onClose={onAdFinished} isRewarded={adType === 'continue'} />}
      <div className="w-full max-w-[450px] mb-4 flex justify-between items-center gap-2">
        <div className="flex-1 min-w-0">
          <h1 className="text-4xl font-black bg-clip-text text-transparent italic tracking-tighter pr-6 overflow-visible drop-shadow-xl select-none" style={{ backgroundImage: 'linear-gradient(to right, #ff0000, #ff7f00, #ffff00, #00ff00, #0000ff, #4b0082, #8b00ff)', WebkitBackgroundClip: 'text' }}>2048STACK</h1>
        </div>
      ...
      <div className="relative">
        <PhysicsGame key={gameId} onUpdateStats={updateStats} isGameOver={stats.isGameOver} onReset={resetGame} continueToken={continueToken} />
        {stats.isGameOver && (
          <div className="absolute inset-0 bg-blue-950/95 backdrop-blur-md flex flex-col items-center justify-center z-50 rounded-xl p-8 text-center animate-in zoom-in duration-300">
            ...
            <div className="flex flex-col gap-4 w-full">
              <button onClick={handleContinueRequest} disabled={isAdLoading} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-500 hover:to-indigo-500 py-5 rounded-2xl font-black text-xl shadow-2xl transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50">
                {isAdLoading && adType === 'continue' ? (
                  <span className="flex items-center gap-2"><i className="fas fa-spinner animate-spin"></i> PREPARING REWARD...</span>
                ) : (
                  <><i className="fas fa-video"></i> CONTINUE (AD REWARD)</>
                )}
              </button>
              <button onClick={resetGame} disabled={isAdLoading && adType === 'periodic'} className="w-full bg-white text-blue-950 hover:bg-slate-200 py-5 rounded-2xl font-black text-xl shadow-xl transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-3 border-4 border-blue-900 disabled:opacity-50">
                {isAdLoading && adType === 'periodic' ? (
                   <span className="flex items-center gap-2"><i className="fas fa-spinner animate-spin"></i> LOADING AD...</span>
                ) : (
